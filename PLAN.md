...
- [x] Follow-up: public-apis sweep (github.com/public-apis/public-apis). Kept the free, keyless lookups that answered in testing: eight more username cards (GitLab, Mastodon, Bluesky, Lichess, Chess.com, Docker Hub, Stack Overflow, crates.io), public records for names (Wikidata, FBI Wanted, CourtListener, NPI, FEC, Nationalize) and companies (Wikidata, ProPublica nonprofits, FEC by employer, NPI), HackerTarget, Cert Spotter and MDN Observatory for domains, ipwho.is and reverse IP for IPs, Kickbox for emails, Wikipedia nearby places and OpenSky aircraft for locations, MAC vendor lookup in the Toolbox. New free keys: abuse.ch, FullHunt, OpenSanctions, Companies House, FEC, CourtListener. Rejected after testing: EVA and Disify (dead or redirecting), SEC full-text search (403 without a registered UA), Blockchair (rate-limits by IP), Reddit (HTML to non-browsers), BGPView (host does not resolve).
